...
describe("plugin timeline projection", () => {
  ...
  it("preserves system messages without exposing them to plugin transforms", () => {
    const source: StreamItem = {
      kind: "system_message",
      id: "system-1",
      timestamp: new Date("2026-01-01T00:00:00.000Z"),
    ...
    const transform: TimelineItemTransform = vi.fn(() => []);

    expect(projectPluginTimelineItems([source], transform)).toEqual([source]);
