...
type DeserializedTimelineBase = Pick<
  Extract<StreamItem, { kind: "assistant_message" }>,
  "id" | "timelineCursor" | "turnId" | "timestamp"
>;
type BasicStoredTimelineItem = Extract<
  StoredTimelineItem,
  { kind: "user_message" | "assistant_message" | "system_message" | "thought" }
>;

const BASIC_TIMELINE_ITEM_KINDS: ReadonlySet<string> = new Set([
  "user_message",
  "assistant_message",
  "system_message",
  "thought",
...
function isBasicStoredTimelineItem(item: StoredTimelineItem): item is BasicStoredTimelineItem {
  return BASIC_TIMELINE_ITEM_KINDS.has(item.kind);
}

function deserializeBasicTimelineItem(
  item: BasicStoredTimelineItem,
  base: DeserializedTimelineBase,
): StreamItem {
  switch (item.kind) {
    case "user_message":
      return {
...
function deserializeTimelineItem(item: StoredTimelineItem): StreamItem {
  const base: DeserializedTimelineBase = {
    id: item.id,
    ...(item.timelineCursor ? { timelineCursor: item.timelineCursor } : {}),
    ...(item.turnId ? { turnId: item.turnId } : {}),
    timestamp: new Date(item.timestamp),
  };
  if (isBasicStoredTimelineItem(item)) {
    return deserializeBasicTimelineItem(item, base);
  }
  switch (item.kind) {
    case "todo_list":
      return {
